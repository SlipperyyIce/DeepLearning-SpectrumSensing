function [bits,decodedInfo,pktValidStatus] = helperBluetoothPracticalReceiver(rxWaveform,rxCfg)
...
btDeviceAddr = comm.internal.utilities.de2biBase2RightMSB(hex2dec(rxCfg.DeviceAddress),48)';
...
[numOfSignals,startIndices,endIndices] = bluetoothSignalIndices(dcCompensatedWaveform,sps);
...
for ii = 1:numOfSignals
    ...
    [outBits,decodedInformation,PktStatus] = bluetoothIdealReceiver(rxComp,rxCfg);
    ...
    if (lapFlag)        
        ...
        validPacketsTemp(count) = PktStatus;
        bits = [bits;outBits];
    end
end
if ~(count == 0)
    decodedInfo = decodedInfoTemp(1:count);
    pktValidStatus = validPacketsTemp(1:count);
else
...
function [numOfSignals,startIndices,endIndices] = bluetoothSignalIndices(waveform,sps)
...
winMag = sum(reshape(abs(waveform(1:nofWindows*windowLen)),windowLen,nofWindows));
for ii = 1:nofWindows
...
for iend = 1:numOfSignals
    ...
    if packetGap<625*sps
       endIndices(iend) = startIndices(iend)+625*sps-1;
   elseif packetGap<3*625*sps
       endIndices(iend) = startIndices(iend)+3*625*sps-1;
   else
       endIndices(iend) = startIndices(iend)+5*625*sps-1;
   end
